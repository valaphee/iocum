...
fn main() {
    ...
    let section = image.section_table().iter().find(|section| section.name.as_slice() == b".data\0\0\0").unwrap();
    ...
    loop {
        ...
        let length = (data[i - 3] as u32 | ((data[i - 2] as u32) << 8) | ((data[i - 1] as u32) << 16)) as usize;
        if length == 0 {
            i += 1;
            continue;
        }
